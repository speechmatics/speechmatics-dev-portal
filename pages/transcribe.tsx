      ...
      <SmPanel width='100%' maxWidth='900px'>
        ...
        <Box py={5} width='100%'>
          ...
          <Flex width='100%' wrap='wrap' gap={6}>
            <SelectField label="Language" tooltip='Expected language of transcription' data={languagesData} />
            <SelectField label="Separation" tooltip='Separation of transcription' data={separation} />
            <ChoiceButtons label="Accuracy" tooltip="Accuracy model" />
...
const SelectField = ({ label, tooltip, data }: SelectFieldProps) => {
  return <Box flex='1 0 40%'>
    ...
    <Select borderColor='smBlack.170' color='smBlack.300' borderRadius='2px' size='lg'>
      {data.map(({ value, label }, i) => <option value={value} selected={i == 0}>{label}</option>)}
    </Select>
  </Box>
}


const ChoiceButtons = ({ label, tooltip }) => {
