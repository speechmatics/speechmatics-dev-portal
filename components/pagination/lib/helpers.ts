...
const union = (...arrays: number[][]) => Array.from(new Set(arrays.flat()));
...
export const generatePages = ({
  ...
  const allPages = Array.from({ length: pagesCount }).map((_, page) => page + 1);
  ...
  const shouldHaveLeftSeparator = getFirstItem(leftInnerPages) > lastPageOfOuterLeftPages + 1;

  const rightInnerPages = generateRightInnerPages();
  const rightPages = union([...rightInnerPages], [...outerRightPages]);
  const shouldHaveRightSeparator = getLastItem(rightInnerPages) < firstPageOfOuterRightPages - 1;
